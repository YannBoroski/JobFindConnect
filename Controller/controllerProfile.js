        ...
        .then((userId) => {
          fetch(`../api/User/apiProfil.php?id=${userId}`, {
            ...
            .then((profileData) => {
              ...
              container.classList.add("container-fluid", "mt-5", "justify-content-center");
              if (profileData.role == "Admin") {
                const buttonWrapper = document.createElement("div");
                buttonWrapper.classList.add("d-flex", "justify-content-center");
              
                const editButton = document.createElement("button");
                editButton.textContent = "Admin Edit";
                editButton.classList.add("btn", "btn-primary", "mt-3");
                editButton.addEventListener("click", () => {
                  window.location.href = "./admin-profile-edit-page.php";
                });
              ...
              Object.entries(profileData).forEach(([key, value]) => {
                ...
                if (key === "pfp") {
                  ...
                  td2 = document.createElement("td");
                  if (value === "User" || value === "Entreprise") {
                    const select = document.createElement("select");
                    select.classList.add("form-control");
                  ...
                  const td3 = document.createElement("td");
                  if (key !== "username" && key !== "isVerified" && key !== "statutVisibilitee" && key !== "date_de_creation" && key !== "last_login" && key !== "pfp" && (key !== "role" || (value !== "User" && value !== "Entreprise"))) {
...
function changeProfile(formDataChange) {
  fetch("../api/User/apiUpdateProfil.php", {
    method: "POST",
    body: formDataChange,
  })
    .then((response) => {
      ...
      .then((userId) => {
        fetch(`../api/Domain/apiDomain.php?id=${userId}`, {
          ...
          .then((domains) => {
            ...
            button.addEventListener("click", linkDomainToUser);
...
function linkDomainToUser() {
  fetch("../api/User/getUserID.php")
    .then((response) => response.text())
    .then((userId) => {
      const domainId = document.getElementById("domain-dropdown").value;
